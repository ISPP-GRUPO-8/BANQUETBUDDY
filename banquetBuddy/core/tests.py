...
class ResetPasswordTests(TestCase):
    ...
    def test_reset_password_view_post_invalid_email(self):
        response = self.client.post(
            reverse("reset_password"), {"email": "nonexistent@example.com"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "core/reset_password.html")
...
class ResetPasswordConfirmTests(TestCase):
    ...
    def test_reset_password_confirm_view_get(self):
        response = self.client.get(
            reverse(
                ...
                kwargs={"token": self.user.reset_password_token},
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_reset_password_confirm_view_post_invalid_token(self):
        response = self.client.post(
            reverse("reset_password_confirm", kwargs={"token": "invalidtoken"}),
            {"password1": "newpassword", "password2": "newpassword"},
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("reset_password"))
...
class ErrorReportTestCase(TestCase):
    ...
    def test_error_report_view_post_particular(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, "/error-report-send")
    ...
    def test_error_report_view_invalid_form(self):
        ...
        self.assertFormError(
            response, "form", "reporter_email", "This field is required."
        )
...
class ListarCateringsHomeTests(TestCase):
    ...
    def test_busqueda_por_nombre_existente(self):
        response = self.client.post(
            reverse("listar_caterings"), {"buscar": "Test Catering Service"}
        )
        self.assertEqual(response.status_code, 200)
...
class NotificationViewTest(TestCase):
    def setUp(self):
        ...
        self.user1 = CustomUser.objects.create_user(
            username="testuser", password="testpassword", email="test1@gmail.com"
        )
        self.user2 = CustomUser.objects.create_user(
            username="testuser2", password="testpassword", email="test2@gmail.com"
        )
        ...
        self.notification1 = NotificationEvent.objects.create(
            user=self.user1, message="Test notification 1", event=self.event
        )
